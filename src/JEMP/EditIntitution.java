...
public class EditIntitution extends JPanel{
	...
	public void search(){
		rs = conn.select("SELECT *FROM INTITUTION WHERE ID_INTITUTION = "+tf.getText());
	...
	public void action(){
		...
		this.b.get(1).addActionListener((e) -> { 
			if(l.get(7).getText() != "n/d"){
				new EditElement("Name","INTITUTION","NAME",l.get(7).getText());
				label();
			}
			else{ JOptionPane.showMessageDialog(null,"No intitution to edit."); }
		});
		
		this.b.get(2).addActionListener((e) -> {
			if(l.get(7).getText() != "n/d"){
				new EditElement("Phone","INTITUTION","PHONE",l.get(7).getText());
				label();
			}
			else{ JOptionPane.showMessageDialog(null,"No intitution to edit."); }
		});
		...
		this.b.get(3).addActionListener((e) -> { 
			if(l.get(7).getText() != "n/d"){
				new EditElement("Id","INTITUTION","ID_INTITUTION",l.get(7).getText());
				label();
			}
			else{ JOptionPane.showMessageDialog(null,"No intitution to edit."); }
		});
		...
		this.b.get(4).addActionListener((e) -> { 
			if(l.get(7).getText() != "n/d"){
				new EditElement("Direction","INTITUTION","DIRECCTION",l.get(10).getText());
				label();
			}
			else{ JOptionPane.showMessageDialog(null,"No intitution to edit."); }
		});
		...
		this.b.get(5).addActionListener((e) -> { 
			if(l.get(7).getText() != "n/d"){
				new EditElement("Supervisor","INTITUTION","ID_SUPERVISOR",l.get(11).getText());
				label();
			}
			else{ JOptionPane.showMessageDialog(null,"No intitution to edit."); }
		});
