...
public class EDIT extends JPanel{
	...
	public void label(String s){
		...
		l.get(0).setText("Edit "+s);
		l.get(1).setText(s+" Id");
		l.get(2).setText("Id:");
	...
	public void panel(String s){
		...
		for(int i=0;i<=12;i++){
			...
			if(i == 12){ p.get(i).setLayout(new GridLayout(2,3)); } 
	...
	public void action(String s){
		this.b.get(0).addActionListener((e) -> { 
			...
			else{ label(s); search(s); text(); }
		});
		
		this.b.get(1).addActionListener((e) -> { 
			if(l.get(4).getText() != "n/d"){
				new EditElement("Id",s,"Id_"+s,l.get(4).getText()); 
				label(s);
			}
			else{ JOptionPane.showMessageDialog(null,"No "+s+" to edit."); }
		});
		
		this.b.get(2).addActionListener((e) -> { 
			if(l.get(4).getText() != "n/d"){
				new EditElement("Desciption",s,"DESCRIPTION",l.get(4).getText());
				label(s);
			}
			else{ JOptionPane.showMessageDialog(null,"No "+s+" to edit."); }
		});
