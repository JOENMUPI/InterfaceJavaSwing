...
public class DeleteProject extends JPanel{
	...
	public void label(){
		...
		l.get(0).setText("Delete project");
	...
	public void button(){
		for(int i=0;i<=2;i++){
			...
			b.get(i).setText("Delete all Projects");
		}
	...
	public void panel(){
		...
		for(int i=0;i<=14;i++){
			...
			if(i == 14){ p.get(i).setLayout(new GridLayout(4,2)); } 

			if(i<=7){p.get(i).add(l.get(i));}
			if(i>7 && i<=10){ p.get(i).add(b.get(j)); j++; }
		}
		
		p.get(11).add(tf);
		
		p.get(14).add(p.get(2));
		p.get(14).add(p.get(5));
		p.get(14).add(p.get(3));
		p.get(14).add(p.get(6));
		p.get(14).add(p.get(4));
		p.get(14).add(p.get(7));
		p.get(14).add(p.get(9));
		p.get(14).add(p.get(10));
		
		p.get(12).add(p.get(1),BorderLayout.NORTH);
		p.get(12).add(p.get(11),BorderLayout.CENTER);;
		
		p.get(13).add(p.get(0),BorderLayout.NORTH);
		p.get(13).add(p.get(12),BorderLayout.CENTER);
		p.get(13).add(p.get(8),BorderLayout.SOUTH);
	}
	...
	public void action(){
		...
		this.b.get(1).addActionListener((e) -> { 
			if(l.get(5).getText() != "n/d"){
				delete("DELETE FROM PROYECT WHERE ID_PROYECT = "+l.get(5).getText());
				label();
			}
			else{ JOptionPane.showMessageDialog(null,"No proyect to delete."); }
		});
		
		this.b.get(2).addActionListener((e) -> { 
			delete("DELETE FROM PROYECT"); 
			label();
		});
	}
